...
举例来说，`http://www.xiaodongsdy.cn/dir/page.html`这个网址，协议是`http://`，域名是`www.xiaodongsdy.cn`，端口是`80`（默认端口可以省略），它的同源情况如下。

- `http://www.xiaodongsdy.cn/dir2/other.html`：同源
- `http://xiaodongsdy.cn/dir/other.html`：不同源（域名不同）
- `http://v2.www.xiaodongsdy.cn/dir/other.html`：不同源（域名不同）
- `http://www.xiaodongsdy.cn:81/dir/other.html`：不同源（端口不同）
- `https://www.xiaodongsdy.cn/dir/page.html`：不同源（协议不同）
...
举例来说，A 网页的网址是`http://w1.xiaodongsdy.cn/a.html`，B 网页的网址是`http://w2.xiaodongsdy.cn/b.html`，那么只要设置相同的`document.domain`，两个网页就可以共享 Cookie。因为浏览器通过`document.domain`属性来检查是否同源。

```javascript
// 两个网页都需要设置
document.domain = 'xiaodongsdy.cn';
```
...
另外，服务器也可以在设置 Cookie 的时候，指定 Cookie 的所属域名为一级域名，比如`xiaodongsdy.cn`。
...
片段标识符（fragment identifier）指的是，URL 的`#`号后面的部分，比如`http://xiaodongsdy.cn/x.html#fragment`的`#fragment`。如果只是改变片段标识符，页面不会重新刷新。
...
window.onload = function () {
  addScriptTag('http://xiaodongsdy.cn/ip?callback=foo');
}
...
上面代码通过动态添加`<script>`元素，向服务器`xiaodongsdy.cn`发出请求。注意，该请求的查询字符串有一个`callback`参数，用来指定回调函数的名字，这对于 JSONP 是必需的。
...
```apl
GET /chat HTTP/1.1
Host: server.xiaodongsdy.cn
Upgrade: websocket
Connection: Upgrade
Sec-WebSocket-Key: x3JJHMbDL1EzLkh9GBhXDw==
Sec-WebSocket-Protocol: chat, superchat
Sec-WebSocket-Version: 13
Origin: http://xiaodongsdy.cn
```
